...
class TestConcurrency:
    ...
    def test_concurrent_writes(self, temp_store):
        ...
        def write_value(key_num):
            try:
                key = f"key{key_num}".encode()
                value = f"value{key_num}".encode()
    ...
    def test_concurrent_read_write(self, temp_store):
        ...
        def writer():
            try:
                for i in range(5):
                    temp_store.put(b"counter", str(i).encode())
                    time.sleep(0.001)
            except Exception as e:
                errors.append(e)
        ...
        def reader():
            try:
                for _ in range(10):
                    value = temp_store.read(b"counter")
                    reads.append(value)
                    time.sleep(0.001)
            except Exception as e:
                errors.append(e)
    ...
    def test_concurrent_deletes(self, temp_store):
        ...
        def delete_key(key_num):
            try:
                key = f"key{key_num}".encode()
                result = temp_store.delete(key)
                results.append(result)
            except Exception as e:
                errors.append(e)
        
        threads = [threading.Thread(target=delete_key, args=(i,)) for i in range(10)]
